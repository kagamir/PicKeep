 ...
 * Supports Mnemonic generation and toSeed conversion
 */
object Bip39 {
    ...
    fun toMnemonic(entropy: ByteArray): List<String> {
        val entropyBits = StringBuilder()
        for (byte in entropy) {
            val binary = String.format("%8s", Integer.toBinaryString(byte.toInt() and 0xFF)).replace(' ', '0')
            entropyBits.append(binary)
        }

        val checksum = calculateChecksum(entropy)
        entropyBits.append(checksum)

        val words = mutableListOf<String>()
        val length = entropyBits.length
        for (i in 0 until length step 11) {
            val indexBin = entropyBits.substring(i, i + 11)
            val index = Integer.parseInt(indexBin, 2)
    ...
    fun toEntropy(words: List<String>): ByteArray {
        require(words.size % 3 == 0) { "Invalid mnemonic length" }
        
        val bits = StringBuilder()
        for (word in words) {
            val index = WORDLIST.indexOf(word)
            require(index != -1) { "Invalid word: $word" }
            val binary = String.format("%11s", Integer.toBinaryString(index)).replace(' ', '0')
            bits.append(binary)
        }

        val dividerIndex = (words.size * 11) - (words.size / 3)
        val entropyBits = bits.substring(0, dividerIndex)
        val checksumBits = bits.substring(dividerIndex)

        val entropyBytes = ByteArray(entropyBits.length / 8)
        for (i in 0 until entropyBytes.size) {
            val byteBin = entropyBits.substring(i * 8, (i + 1) * 8)
            entropyBytes[i] = Integer.parseInt(byteBin, 2).toByte()
        }

        val calculatedChecksum = calculateChecksum(entropyBytes)
        require(checksumBits == calculatedChecksum) { "Invalid checksum" }
    ...
    fun toSeed(mnemonic: List<String>, passphrase: String = ""): ByteArray {
        ...
        // Android API 26+ supports PBKDF2WithHmacSHA512
        val factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA512")
        return factory.generateSecret(spec).encoded
    }

    private fun calculateChecksum(entropy: ByteArray): String {
        val ent = entropy.size * 8
        val cs = ent / 32
        
        val md = MessageDigest.getInstance("SHA-256")
        val hash = md.digest(entropy)
        val firstByte = hash[0].toInt()
        
        val binary = String.format("%8s", Integer.toBinaryString(firstByte and 0xFF)).replace(' ', '0')
        return binary.substring(0, cs)
    }
    ...
    val WORDLIST = listOf(
        ...
        "debate", "debris", "debt", "decade", "december", "decide", "deck", "decrease", "dedicate", "deer",
        ...
        "job", "join", "joke", "journey", "joy", "judge", "juggle", "juice", "jump", "jungle",
        ...
        "since", "sing", "siren", "sister", "sit", "situation", "size", "skate", "sketch", "ski",
